      ...
      <div className="h-24 flex items-end gap-2">
        {totals.map((v, i) => {
          ...
          return (
            <div
              key={months[i]}
              className="h-full flex-1 flex flex-col items-center gap-1 group"
            >
              <div className="flex-1 w-full flex items-end">
                <div
                  className={`w-full rounded-t origin-bottom transition-[height,transform,box-shadow,filter] duration-500 ease-out group-hover:scale-y-[1.04] group-hover:shadow-lg ${
                    isActive
                      ? 'bg-gradient-to-t from-accent to-[#ff7b73] shadow-accent/30'
                      : 'bg-gradient-to-t from-[#4fc3f7] to-[#81d4fa]'
