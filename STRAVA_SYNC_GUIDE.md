...
| **strava_sync.py** | 简单同步脚本 | 日常增量同步（最近7天） |
...
| 限制类型 | 配额 | 说明 |
|---------|------|------|
| 15 分钟窗口 | 600 次调用 | 滑动窗口限制 |
| 每日总量 | 30,000 次调用 | 自然日限制 |
...
|------|--------|------|
| 批次大小 | 150 个活动 | 自动计算 |
| 批次间隔 | 900 秒（15 分钟） | 可自定义 |
| 每批次 API 调用 | ~600 次 | 刚好达到 15 分钟限制 |
